...
namespace GraphColoring{
    class Lmxrlf : public GraphColor {
        ...
        public: 
            Lmxrlf() : GraphColor() { this->condition = 0; }
            Lmxrlf(map<string, vector<string> > input_graph) :GraphColor(input_graph) { this->condition = 0; } 
            Lmxrlf(map<string, vector<string> > input_graph, int con) :GraphColor(input_graph){ this->condition = con; }
